...
def unpaid(request):
    ...
    if request.method == 'POST':
        if 'name' in request.POST:
            ...
            page = request.GET.get('page')
            list = paginator.get_page(page)
    return render (request, 'cashier/unpaid_consultation.html',{"list":list,'select_name' : name })
...
def unpaid_exam(request):
    ...
    if request.method == 'POST':
        if 'name' in request.POST:
            name = request.POST['name']
            list = ExamRequest.objects.filter(idPatient__contains = name)
            paginator = Paginator(list, 9)
            page = request.GET.get('page')
            list = paginator.get_page(page)
...
def display(request,id):
    consultation = Consultation.objects.get(id=id)
    patient = Patient.objects.get(firstName = consultation.idPatient)
    ...
    staff= MedicalStaff.objects.get(username = consultation.idMedicalStaff)
...
def display_exam(request,id):
    ...
    patient = Patient.objects.get(firstName= examRequest.idPatient)
    staff= MedicalStaff.objects.get(username = examRequest.idMedicalStaff)
    exam = Exam.objects.get(examName = examRequest.idExam)

    return render(request, 'cashier/display_exam.html', {'exam': exam,'patient':patient,'staff':staff,'examRequest':examRequest})
