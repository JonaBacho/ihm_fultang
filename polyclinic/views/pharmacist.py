...
def add_medicament(request):
    try :
        ...
        list = paginator.get_page(paginator.num_pages)
    except : 
        message = "restart the operation, go to add new Medicament"
        return render(request,'pharmacist/404.html',{'message':message})  

    return render(request, 'pharmacist/medicament_list.html', {'list': list})

def edit (request,id):
    try:
        medicament = Medicament.objects.get(id=id)
    except :
        message = "corruption, bill not found"
        return render(request,'pharmacist/404.html',{'message':message})  
...
def save_bill (request,id) :
    ...
    try : 
        ...
        for i in range(len(items)):
            ...
            if med.quantity<1 : 
                message = 'invalid quantity for '+med.medicamentName+'restart the bill'
                return render(request,'pharmacist/404.html', {'message' : message})
            med.save()   
        ...
        bill.delete()
        message = 'error you must have SAVE before submit, restart the bill'
        return render(request,'pharmacist/404.html', {'message' : message})
...
def bills(request):
    ...
    if request.method == 'POST':
        if 'name' in request.POST:
            ...
            page = request.GET.get('page')
            bills = paginator.get_page(page)
    return render (request, 'pharmacist/bills.html',{"list":bills,'select_name' : name })
